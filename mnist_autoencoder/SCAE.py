...
class KWinners(nn.Module):
    ...
    def __init__(self, n, k, kInferenceFactor=1.0, boostStrength=1.0,
        ...
        self.register_buffer("dutyCycle", torch.zeros((1, n, 1, 1)))
    ...
    def entropy(self):
        ...
        if self.k < self.n:
            ...
            entropy[x * (1 - x) == 0] = 0
            return entropy.sum() * self.n / self.channels
        else:
            return 0
...
class Autoencoder(nn.Module):
    ...
    def __init__(self, width, height, n, k, boost_strength):
        self.width = width
        self.height = height
        super(Autoencoder, self).__init__()
        self.conv1 = nn.Conv2d(1, 8, kernel_size=5)
        self.conv2 = nn.Conv2d(8, n, kernel_size=5)
        self.k_winners = KWinners(n, k, boostStrength=boost_strength)
        self.conv3 = nn.ConvTranspose2d(n, 8, kernel_size=5)
        self.conv4 = nn.ConvTranspose2d(8, 1, kernel_size=5)
...
EPOCHS = 1000
model = Autoencoder(28, 28, 32, 2, 1.0).to(DEVICE)
distance = nn.MSELoss()
