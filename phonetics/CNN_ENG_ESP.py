...
def download(f):
    if not os.path.isfile(f):
        ...
        open(f, 'wb').write(
            requests.get('https://raw.githubusercontent.com/open-dict-data/ipa-dict/master/data/' + f).content)


download(DATA_FILE_EN)
download(DATA_FILE_ES)
...
def tsne_show(vectors, labels):
    ...
    def update_annot(ind):
        ...
        annot.xy = pos
        text = ", ".join([' '.join(labels[n]) for n in ind["ind"]])
        annot.set_text(text)
