...
class ResnetRNN(nn.Module):
    ...
    def __init__(self, rnn_hidden):
        ...
        self.lin1 = nn.Linear(1024, rnn_hidden)
        self.rnn_init = nn.Parameter(torch.randn(1, rnn_hidden))
        self.rnn = nn.Linear(rnn_hidden * 2, len(alphabet) + 1 + rnn_hidden)
    ...
    def forward(self, x):
        ...
        h = self.rnn_init.repeat(BATCH_SIZE, 1)
        output = []
...
def train(visualize):
    ...
    for epoch in range(EPOCHS):
        ...
        # output.size() == (BATCH_SIZE, MAX_LEN, len(alphabet)+1)
